...
The United States is more productive than Bangladesh in \emph{both}
textiles \emph{and} aircraft.

Yet the US imports textiles from Bangladesh.

\textbf{Why?} → Enter David Ricardo (1817).
\end{column}
\end{columns}
\end{frame}
...
A lawyer who types faster than her secretary still benefits from hiring
the secretary --- because the lawyer's time is more valuably spent on
legal work.
...
\textbf{Assume:} each country has \(L = 100\) workers.
...
The \textbf{World} has absolute advantage in \textbf{both} goods (lower
\(a\)).
\end{column}
...
To produce one more unit of \textbf{Rice}, India must give up:
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, breakable, opacitybacktitle=0.6, left=2mm, opacityback=0, colframe=quarto-callout-note-color-frame, coltitle=black, rightrule=.15mm, colback=white, bottomrule=.15mm, toptitle=1mm, bottomtitle=1mm, titlerule=0mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Key takeaway}, leftrule=.75mm, toprule=.15mm]
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture04_Comparative_Advantage_files/figure-beamer/fig-ppf-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture04_Comparative_Advantage_files/figure-beamer/fig-gains-1.pdf}}
...
India produces and consumes on the PPF, say at: \[(Wh, R) = (50, 100)\]

\textbf{After trade} (at \(P_{Wh}/P_R = 1.5\)):

India fully specialises → 200 Rice, trades \(107\) Rice for
\(\approx 80\) Wheat:
...
The \textbf{real wage} of Indian workers rises: each hour of labour now
commands more consumption goods.
\end{column}
...
The trade budget line is \textbf{flatter} → larger feasible set.
\end{column}
\end{columns}
\end{frame}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture04_Comparative_Advantage_files/figure-beamer/fig-ic-ppf-1.pdf}}
...
Golub \& Hsieh (2000) and Costinot et al.~(2012) find strong evidence
that labour productivity differences predict export patterns across
countries.
...
\item
  Says nothing about \emph{who within a country} gains or loses
\item
...
\tightlist
\item
  \textbf{Many goods:} a continuum of goods ranked by comparative
  advantage --- a ``Dornbusch-Fischer-Samuelson'' chain
\item
  \textbf{Many countries:} bilateral comparisons still apply
\item
  \textbf{Technology changes:} productivity growth shifts the chain,
  altering trade patterns over time
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture04_Comparative_Advantage_files/figure-beamer/fig-rca-1.pdf}}
...
\textbf{Why does Ricardo's model fit?}
...
\item
  Labour productivity in Indian rice farming (output per worker) is
  competitive even though absolute productivity lags behind
  Vietnam/Thailand
\item
...
\textbf{Policy tension:}

September 2023: India imposed a \textbf{25\% export duty} on non-Basmati
white rice to contain domestic inflation → global rice prices spiked
15\%.
\end{column}

\begin{column}{0.45\linewidth}
\textbf{Ricardian lens on the export ban:}
...
\textbf{This is the central policy tension in agricultural trade:}
...
\textbf{What Ricardo cannot explain:}

\begin{itemize}
\tightlist
\item
  \emph{Why} labour productivity differs (→ H-O model)
\item
...
\textbf{Next lecture:} Where do comparative advantages come from? Factor
