...
shorttitle: "L6 | Terms of Trade & FTAs"
...
## ToT Calculation: India's Actual Data {.smaller}

$$\text{ToT} = \frac{P_x}{P_m} \times 100 \qquad P_x = \text{agri export price index (2015=100)}, \quad P_m = \text{oil import price index (2015=100)}$$

```{r}
#| label: tbl-tot-calc
#| tbl-cap: "India: Agricultural ToT — Worked Calculations for Selected Years (oil as import-price proxy)"
...
data.frame(
  ...
  dplyr::mutate(
    Pm  = round(oil_price / oil_price[year == 2015] * 100),
    Px  = round(tot_agri  * Pm / 100),
    ToT = round(Px / Pm * 100),
  ...
  dplyr::filter(year %in% c(2010, 2014, 2015, 2016, 2020, 2022, 2024)) |>
  dplyr::select(
    Year           = year,
    "Px (Agri Export)" = Px,
    "Pm (Oil Import)"  = Pm,
...
::: {.fragment}
::: {.hi-gold}
**Key insight:** In 2016 and 2020, oil crashed — *Pm* fell sharply — yet ToT barely improved, because agri export prices (*Px*) fell in tandem. This asymmetry is Prebisch-Singer in the data.
:::
:::
...
india_tot <- data.frame(
  ...
  # Normalise oil price to 2015=100 so both series share the same index scale
  dplyr::mutate(oil_idx = oil_price / oil_price[year == 2015] * 100)

ggplot(india_tot, aes(x = year)) +
  geom_line(aes(y = tot_agri, colour = "Agri ToT (2015=100)"),
            linewidth = 1.5) +
  geom_line(aes(y = oil_idx, colour = "Brent crude (2015=100)"),
            linewidth = 1.2, linetype = "dashed") +
  geom_hline(yintercept = 100, linetype = "dotted", colour = "grey60") +
  annotate("text", x = 2010.5, y = 104, label = "Base = 100", size = 3, colour = "grey50") +
  scale_colour_manual(
    values = c("Agri ToT (2015=100)"    = "#012169",
               "Brent crude (2015=100)" = "#B9975B")) +
  scale_x_continuous(breaks = 2010:2024) +
  labs(title    = "India: Agricultural Terms of Trade vs Oil Prices (2010–2024)",
       subtitle = "Both series indexed to 2015=100; oil is far more volatile than agri ToT",
       x = NULL, y = "Index (2015 = 100)", colour = NULL) +
  theme(legend.position = "bottom",
