...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture02_Features_files/figure-beamer/fig-trade-gdp-1.pdf}}
...
\textbf{(a) Geographical separation} Long distances → higher transport
costs, insurance, cold-chain requirements, longer lead times
...
\begin{column}{0.5\linewidth}
\textbf{(d) Multiple political jurisdictions} Each country sets its own
trade policy --- tariffs, quotas, export bans; no single governing
authority

\textbf{(e) Different legal systems \& culture} Food labelling laws,
packaging standards, contract enforcement vary; Indian spice exporters
must comply with EU pesticide MRL standards

\textbf{(f) Greater risk and uncertainty} Combination of all above →
higher commercial, political, and exchange-rate risk than domestic
commerce
\end{column}
\end{columns}
\end{frame}
...
\item
  ...
  \item
    ...
    but under strict bilateral agreements, not free movement
  \end{itemize}
\item
...
\textbf{India case: Gulf remittances}

Indian workers (largely from Kerala, UP, Bihar) work in UAE, Saudi
Arabia, Kuwait --- sending back \textbf{\textasciitilde\$120 billion in
remittances (FY2024)}, making India the world's largest remittance
recipient.

This is \emph{partial} factor mobility --- workers move but face
fixed-term contracts and limited rights.

Remittances are India's largest single source of foreign exchange ---
larger than IT exports or any single merchandise export category.
\end{column}
\end{columns}
\end{frame}
...
\begin{column}{0.45\linewidth}
\textbf{Agricultural export example}

A Punjab wheat exporter quotes \textbf{\$250/tonne} FOB Mumbai.
...
A \textbf{5\% INR appreciation = 6.25\% reduction} in rupee revenue ---
potentially wiping out margins on thin-traded commodities.
...
Where: - \(T_{ij}\) = trade between country \(i\) and country \(j\) -
\(Y_i, Y_j\) = GDP (economic size) of each country - \(D_{ij}\) =
...
\begin{column}{0.5\linewidth}
\textbf{Political and structural risks}

\begin{enumerate}
\setcounter{enumi}{4}
\tightlist
\item
  \textbf{Economic dependency} --- Over-specialisation creates
  vulnerability (e.g., countries heavily dependent on single commodity
  exports)
\item
  \textbf{Trade disputes} --- India has faced WTO challenges on sugar
  subsidies, poultry import bans
\item
  \textbf{Structural unemployment} --- Import competition can displace
...
\item
  Global edible oil prices rose \textbf{50--80\%} in 6 months
\item
  India's retail edible oil inflation peaked at \textasciitilde20\%
  mid-2022
\item
...
\textbf{Who gained from the ban:} - Urban consumers (domestic rice
prices stabilised) - State governments managing food inflation - RBI's
CPI inflation target

\textbf{Who lost:} - Paddy farmers --- domestic price fell when export
outlet closed - Rice millers and exporters --- contracts cancelled;
reputation damage - Bangladesh, West Africa --- importers faced supply
shortage; global rice prices rose +20\%
\end{column}

\begin{column}{0.4\linewidth}
\textbf{Policy outcome:}
...
India partially reversed the ban for specific grades in early 2024 under
WTO member pressure.
...
\begin{column}{0.45\linewidth}
\textbf{Why small agri-exporters struggle}

Indian rice exporters to EU must meet \textbf{Maximum Residue Levels
(MRLs)} for 300+ pesticides. Failure → entire consignment rejected at
port.

L/C requires strong banking relationships. MSME spice and horticulture
exporters often cannot negotiate L/C terms with large US/EU importers.

\textbf{ECGC} provides credit insurance --- covers non-payment risk for
Indian exporters.
\end{column}
\end{columns}
\end{frame}
...
\item
  ...
  (2024) --- a dramatic shift driven by 1991 liberalisation
\item
...
\item
  ...
  agricultural import vulnerability: \$12--14B/year, 60--65\% of
  consumption
\item
