...
- Each country exports the good intensive in its abundant factor; both gain welfare
- Production shifts from **A/A\*** (autarky) → **B/B\*** (trade); consumption reaches **E/E\*** on higher IC
...
# ── INDIA (Home, Labor-abundant) ─────────────────────────────────────────────
# PPF: R = 200√(1-(M/100)²)  |  slope = -M/(50√(1-(M/100)²))
# Autarky A: slope=-2  →  M=50√2≈70.71,  R=100√2≈141.42
# Production B: slope=-1.5  →  M=60,  R=160
# World price line Pb: R = 250-1.5M  (through B)
# Consumption E: MRS=1.5 on Pb  →  M=250/3≈83.33,  R=125
A_M <- 50*sqrt(2);  A_R <- 100*sqrt(2)
B_M <- 60;          B_R <- 160
E_M <- 250/3;       E_R <- 125
U1i <- A_M * A_R                  # ≈ 10000
U2i <- E_M * E_R                  # ≈ 10417

m_i   <- seq(0.5, 99.5, len = 400)
m_seq <- seq(4, 116, len = 500)

p_india <- ggplot() +
  # PPF
  geom_line(data = data.frame(M = m_i, R = 200*sqrt(1-(m_i/100)^2)),
            aes(M, R), color = "#012169", linewidth = 1.4) +
  # Pa (autarky price, slope -2 through A)
  geom_line(data = data.frame(M = m_seq,
                              R = pmax(0, A_R + 2*(A_M - m_seq))) |>
              subset(R >= 0 & R <= 228),
            aes(M, R), color = "grey45", linewidth = 0.8, linetype = "dotted") +
  # Pw (world price, slope -1.5 through B and E)
  geom_line(data = data.frame(M = m_seq, R = pmax(0, 250 - 1.5*m_seq)) |>
              subset(R <= 228),
            aes(M, R), color = "#C8952A", linewidth = 1.0, linetype = "dashed") +
  # IC1 through A, IC2 through E
  geom_line(data = data.frame(M = m_seq, R = pmin(U1i/m_seq, 228)) |>
              subset(R >= 0 & R <= 228),
            aes(M, R), color = "grey55", linewidth = 0.8, linetype = "dotted") +
  geom_line(data = data.frame(M = m_seq, R = pmin(U2i/m_seq, 228)) |>
              subset(R >= 0 & R <= 228),
            aes(M, R), color = "#2E7D32", linewidth = 0.8, linetype = "dotted") +
  # Points
  geom_point(aes(A_M, A_R), color = "#012169", size = 2.5) +
  geom_point(aes(B_M, B_R), color = "#012169", size = 2.5) +
  geom_point(aes(E_M, E_R), color = "#C8952A", size = 2.5) +
  # Trade triangle
  annotate("segment", x=B_M, xend=E_M, y=B_R, yend=B_R,
           color="#2E7D32", linewidth=0.7,
           arrow=arrow(length=unit(0.12,"cm"), ends="last")) +
  annotate("segment", x=E_M, xend=E_M, y=B_R, yend=E_R,
           color="#2E7D32", linewidth=0.7,
           arrow=arrow(length=unit(0.12,"cm"), ends="first")) +
  # Point labels
  annotate("text", x=A_M+4,  y=A_R+10, label="A",  fontface="bold", size=3.2, color="#012169") +
  annotate("text", x=B_M-8,  y=B_R+9,  label="B",  fontface="bold", size=3.2, color="#012169") +
  annotate("text", x=E_M+3,  y=E_R-11, label="E",  fontface="bold", size=3.2, color="#C8952A") +
  # Price line labels
  annotate("text", x=113, y=250-1.5*113+7,       label="P[w]", parse=TRUE, size=2.9, color="#C8952A", hjust=0) +
  annotate("text", x=107, y=A_R+2*(A_M-107)-8,   label="P[a]", parse=TRUE, size=2.9, color="grey45", hjust=0) +
  # IC labels (placed near tangency region, not clipped)
  annotate("text", x=76, y=U2i/76+7,  label="IC[2]", parse=TRUE, size=2.6, color="#2E7D32") +
  annotate("text", x=76, y=U1i/76-8,  label="IC[1]", parse=TRUE, size=2.6, color="grey55") +
  # Trade annotations
  annotate("text", x=(B_M+E_M)/2, y=B_R+10,    label="Imports M", size=2.4, hjust=0.5, color="#2E7D32") +
  annotate("text", x=E_M+3,       y=(B_R+E_R)/2, label="Exports R", size=2.4, hjust=0,   color="#2E7D32") +
  scale_x_continuous(limits = c(0, 118), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 228), expand = c(0,0)) +
  labs(title = "India (Labor-Abundant)", x = "Machines (M)", y = "Rice (R)") +
  th

# ── USA (Foreign, Capital-abundant) ──────────────────────────────────────────
# PPF: R = 100√(1-(M/200)²)  |  slope = -M/(400√(1-(M/200)²))
# Autarky A*: slope=-0.5  →  M=100√2≈141.42,  R=100/√2≈70.71
# Production B*: slope=-1.5  →  M=60√10≈189.74,  R≈31.62
# World price line Pw: R = 316.23-1.5M  (through B*)
# Consumption E*: MRS=1.5 on Pw  →  M≈105.41,  R≈158.12
As_M <- 100*sqrt(2);  As_R <- 100/sqrt(2)
Bs_M <- 60*sqrt(10);  Bs_R <- 100*sqrt(1-(Bs_M/200)^2)
Pw_int <- Bs_R + 1.5*Bs_M
Es_M <- Pw_int/3;     Es_R <- 1.5*Es_M
U1u <- As_M * As_R                 # ≈ 10000
U2u <- Es_M * Es_R                 # ≈ 16665

m_u <- seq(0.5, 199.5, len = 400)
m_us <- seq(4, 215, len = 600)

p_usa <- ggplot() +
  # PPF
  geom_line(data = data.frame(M = m_u, R = 100*sqrt(1-(m_u/200)^2)),
            aes(M, R), color = "#B22234", linewidth = 1.4) +
  # Pa* (autarky price, slope -0.5 through A*)
  geom_line(data = data.frame(M = m_us,
                              R = pmax(0, As_R + 0.5*(As_M - m_us))) |>
              subset(R >= 0 & R <= 172),
            aes(M, R), color = "grey45", linewidth = 0.8, linetype = "dotted") +
  # Pw (world price, slope -1.5 through B* and E*)
  geom_line(data = data.frame(M = m_us, R = pmax(0, Pw_int - 1.5*m_us)) |>
              subset(R <= 172),
            aes(M, R), color = "#C8952A", linewidth = 1.0, linetype = "dashed") +
  # IC1* through A*, IC2* through E*
  geom_line(data = data.frame(M = m_us, R = pmin(U1u/m_us, 172)) |>
              subset(R >= 0 & R <= 172),
            aes(M, R), color = "grey55", linewidth = 0.8, linetype = "dotted") +
  geom_line(data = data.frame(M = m_us, R = pmin(U2u/m_us, 172)) |>
              subset(R >= 0 & R <= 172),
            aes(M, R), color = "#2E7D32", linewidth = 0.8, linetype = "dotted") +
  # Points
  geom_point(aes(As_M, As_R), color = "#B22234", size = 2.5) +
  geom_point(aes(Bs_M, Bs_R), color = "#B22234", size = 2.5) +
  geom_point(aes(Es_M, Es_R), color = "#C8952A", size = 2.5) +
  # Trade triangle (USA exports machines, imports rice)
  annotate("segment", x=Es_M, xend=Bs_M, y=Bs_R, yend=Bs_R,
           color="#2E7D32", linewidth=0.7,
           arrow=arrow(length=unit(0.12,"cm"), ends="first")) +
  annotate("segment", x=Es_M, xend=Es_M, y=Bs_R, yend=Es_R,
           color="#2E7D32", linewidth=0.7,
           arrow=arrow(length=unit(0.12,"cm"), ends="last")) +
  # Point labels
  annotate("text", x=As_M+5,  y=As_R+9,  label="A*", fontface="bold", size=3.2, color="#B22234") +
  annotate("text", x=Bs_M+5,  y=Bs_R-9,  label="B*", fontface="bold", size=3.2, color="#B22234") +
  annotate("text", x=Es_M-14, y=Es_R+8,  label="E*", fontface="bold", size=3.2, color="#C8952A") +
  # Price line labels
  annotate("text", x=204, y=Pw_int-1.5*204+6,          label="P[w]",         parse=TRUE, size=2.9, color="#C8952A", hjust=0) +
  annotate("text", x=198, y=As_R+0.5*(As_M-198)-8,     label="P[a]^'*'",     parse=TRUE, size=2.9, color="grey45", hjust=0) +
  # IC labels (near tangency region where curves are visible)
  annotate("text", x=155, y=U2u/155+7,  label="IC[2]^'*'", parse=TRUE, size=2.6, color="#2E7D32") +
  annotate("text", x=155, y=U1u/155-8,  label="IC[1]^'*'", parse=TRUE, size=2.6, color="grey55") +
  # Trade annotations
  annotate("text", x=(Bs_M+Es_M)/2, y=Bs_R-11,   label="Exports M", size=2.4, hjust=0.5, color="#2E7D32") +
  annotate("text", x=Es_M-4,        y=(Bs_R+Es_R)/2, label="Imports R", size=2.4, hjust=1, color="#2E7D32") +
  scale_x_continuous(limits = c(0, 215), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, 172), expand = c(0,0)) +
  labs(title = "USA (Capital-Abundant)", x = "Machines (M)", y = "Rice (R)") +
  th
