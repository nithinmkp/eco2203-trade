...
**Free trade (world price $P_b$, slope $= -1.5$):**

- Production shifts to **B** $(M=60,\, R=160)$
- India exports **35 Rice**, imports **23 Machines**
- Consumption at **E** $(M=83,\, R=125)$ on IC2

**IC2 $>$ IC1 → welfare gain**

*Adapted from Salvatore, Fig. 5.4*
:::
::: {.column width="60%"}
```{r}
#| label: fig-ho-welfare
#| fig-cap: "Basis for and Gains from Trade under the H-O Model (after Salvatore Fig. 5.4)"
#| fig-width: 6.5
#| fig-height: 5
library(ggplot2)

# PPF: R = 200*sqrt(1-(M/100)^2) — concave H-O frontier
M_ppf <- seq(0.5, 99.5, by=0.5)
R_ppf <- 200 * sqrt(1 - (M_ppf/100)^2)

# Key points (derived from PPF slope conditions)
A_M <- 100/sqrt(2); A_R <- 200/sqrt(2)   # autarky: PPF slope = -2
B_M <- 60;          B_R <- 160            # production: PPF slope = -1.5
E_M <- 250/3;       E_R <- 125            # consumption: MRS = 1.5 on Pb line

# Price lines
M_pa <- seq(20, 115, by=0.5)
R_pa <- 282.8 - 2*M_pa                    # Pa: autarky price, slope -2 through A

M_pb <- seq(0, 115, by=0.5)
R_pb <- pmax(0, 250 - 1.5*M_pb)           # Pb: world price, slope -1.5 through B & E

# Indifference curves: U = sqrt(R*M) → R = U^2/M
M_ic <- seq(5, 115, by=0.5)
R_ic1 <- pmin(10000 / M_ic, 235)          # IC1 through A  (U^2=10000)
R_ic2 <- pmin(10417 / M_ic, 235)          # IC2 through E  (U^2=10417)

df_ppf <- data.frame(M=M_ppf, R=R_ppf)
df_pa  <- data.frame(M=M_pa,  R=R_pa)
df_pb  <- data.frame(M=M_pb,  R=R_pb)
df_ic1 <- data.frame(M=M_ic,  R=R_ic1)
df_ic2 <- data.frame(M=M_ic,  R=R_ic2)

ggplot() +
  # PPF
  geom_line(data=df_ppf, aes(x=M, y=R), color="#012169", linewidth=1.8) +
  # Pa: domestic autarky price line (dotted)
  geom_line(data=subset(df_pa, R>=0 & R<=235),
            aes(x=M, y=R), color="grey40", linewidth=0.9, linetype="dotted") +
  # Pb: world price / trade possibilities line (dashed)
  geom_line(data=subset(df_pb, R<=235),
            aes(x=M, y=R), color="#B9975B", linewidth=1.2, linetype="dashed") +
  # IC1 (autarky) and IC2 (free trade)
  geom_line(data=subset(df_ic1, M>=8  & R<=235), aes(x=M, y=R),
            color="grey55", linewidth=0.9, linetype="dotted") +
  geom_line(data=subset(df_ic2, M>=8  & R<=235), aes(x=M, y=R),
            color="darkgreen", linewidth=0.9, linetype="dotted") +
  # Points A, B, E
  geom_point(aes(x=A_M, y=A_R), color="#012169",  size=3.5) +
  geom_point(aes(x=B_M, y=B_R), color="#012169",  size=3.5) +
  geom_point(aes(x=E_M, y=E_R), color="#B9975B",  size=3.5) +
  # Trade triangle
  annotate("segment", x=B_M, xend=E_M, y=B_R, yend=B_R,
           color="darkgreen", linewidth=0.8, linetype="solid",
           arrow=arrow(length=unit(0.18,"cm"), ends="last")) +
  annotate("segment", x=E_M, xend=E_M, y=B_R, yend=E_R,
           color="darkgreen", linewidth=0.8, linetype="solid",
           arrow=arrow(length=unit(0.18,"cm"), ends="last")) +
  # Point labels
  annotate("text", x=A_M+2, y=A_R+10, label="A", fontface="bold", size=4, color="#012169") +
  annotate("text", x=B_M-8, y=B_R+8,  label="B", fontface="bold", size=4, color="#012169") +
  annotate("text", x=E_M+2, y=E_R-12, label="E", fontface="bold", size=4, color="#B9975B") +
  # Price line labels
  annotate("text", x=112, y=282.8-2*112-4, label="P[a]", parse=TRUE,
           size=3.5, color="grey40", hjust=0) +
  annotate("text", x=112, y=250-1.5*112+6, label="P[b]", parse=TRUE,
           size=3.5, color="#B9975B", hjust=0) +
  # IC labels
  annotate("text", x=9, y=10417/9-10, label="IC[2]", parse=TRUE,
           size=3, color="darkgreen", hjust=0) +
  annotate("text", x=9, y=10000/9+6,  label="IC[1]", parse=TRUE,
           size=3, color="grey55",    hjust=0) +
  # Trade annotations
  annotate("text", x=(B_M+E_M)/2, y=B_R+9,
           label="Imports Machines", size=2.8, hjust=0.5, color="darkgreen") +
  annotate("text", x=E_M+2, y=(B_R+E_R)/2,
           label="Exports Rice", size=2.8, hjust=0, color="darkgreen") +
  scale_x_continuous(limits=c(0, 118)) +
  scale_y_continuous(limits=c(0, 235)) +
  labs(x="Machines (capital-intensive)", y="Rice (labour-intensive)") +
  theme_minimal(base_size=11) +
