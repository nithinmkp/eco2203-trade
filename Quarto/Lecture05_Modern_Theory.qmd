...
# ── Coordinates (Salvatore convention: X = labor-intensive, Y = capital-intensive)
# Axes: X = Rice (labor-intensive),  Y = Machines (capital-intensive)
#
# India (labor-abundant): PPF  M = 100√(1−(R/200)²)  [biased toward Rice]
#   dM/dR = −R / (400√(1−(R/200)²))
#   Pa slope −0.5  →  A  = (100√2, 100/√2)  ≈ (141.4,  70.7)
#   Pw slope −1    →  B  = (80√5,  20√5)    ≈ (178.9,  44.7)  [more Rice, fewer Machines]
#   Consumption    →  E  = (50√5,  50√5)    ≈ (111.8, 111.8)  [MRS=1 on Pw, ABOVE PPF]
#
# USA  (capital-abundant): PPF  M = 200√(1−(R/100)²)  [biased toward Machines]
#   dM/dR = −R / (50√(1−(R/100)²))
#   Pa* slope −2   →  A* = (50√2,  100√2)  ≈ ( 70.7, 141.4)
#   Pw slope −1    →  B* = (20√5,  80√5)   ≈ ( 44.7, 178.9)  [fewer Rice, more Machines]
#   Consumption    →  E* = (50√5,  50√5)   ≈ (111.8, 111.8)  [symmetric example]
#
# Pw:  M = 100√5 − R  (same line for both; intercept 223.6)
# IC utility: R×M = constant  →  M = U/R
# India:  U1=10000  (IC1 through A),  U2=12500 (IC2 through E)
# USA:    U1=10000  (IC1* through A*), U2=12500 (IC2* through E*)

A_R  <- 100*sqrt(2);  A_M  <- 100/sqrt(2)   # India autarky
B_R  <- 80*sqrt(5);   B_M  <- 20*sqrt(5)    # India production under Pw
E_R  <- 50*sqrt(5);   E_M  <- 50*sqrt(5)    # India & USA consumption (symmetric)
Pw_c <- 100*sqrt(5)                          # Pw: M = Pw_c − R
U1   <- 10000;        U2   <- 12500

As_R <- 50*sqrt(2);   As_M <- 100*sqrt(2)   # USA autarky
Bs_R <- 20*sqrt(5);   Bs_M <- 80*sqrt(5)    # USA production under Pw

ri  <- seq(0.5, 225, len = 600)   # R grid for India panel
ru  <- seq(0.5, 122, len = 600)   # R grid for USA panel

# ── India panel ───────────────────────────────────────────────────────────────
p_india <- ggplot() +
  # PPF  (M = 100√(1−(R/200)²))
  geom_line(data = data.frame(R = seq(0.5,200,len=400),
                              M = 100*sqrt(1-(seq(0.5,200,len=400)/200)^2)),
            aes(R, M), color = "#012169", linewidth = 1.4) +
  # Pa: M = 100√2 − 0.5R  (slope −0.5, rice is cheap at autarky)
  geom_line(data = data.frame(R = ri, M = pmax(0, 100*sqrt(2) - 0.5*ri)),
            aes(R, M), color = "grey45", linewidth = 0.8, linetype = "dotted") +
  # Pw: M = Pw_c − R  (slope −1, world price)
  geom_line(data = data.frame(R = ri, M = pmax(0, Pw_c - ri)),
            aes(R, M), color = "#C8952A", linewidth = 1.0, linetype = "dashed") +
  # IC1 (through A) and IC2 (through E)
  geom_line(data = data.frame(R = ri, M = U1/ri), aes(R, M),
            color = "grey55", linewidth = 0.8, linetype = "dotted") +
  geom_line(data = data.frame(R = ri, M = U2/ri), aes(R, M),
            color = "#2E7D32", linewidth = 0.8, linetype = "dotted") +
  # Points A, B, E
  geom_point(aes(A_R, A_M), color = "#012169", size = 2.8) +
  geom_point(aes(B_R, B_M), color = "#012169", size = 2.8) +
  geom_point(aes(E_R, E_M), color = "#C8952A", size = 2.8) +
  # Trade triangle:
  #   India EXPORTS Rice  → horizontal arrow from B leftward to E_R
  #   India IMPORTS Machines → vertical arrow from B_M upward to E_M
  annotate("segment", x=B_R, xend=E_R, y=B_M, yend=B_M,
           color="#2E7D32", linewidth=0.8,
           arrow=arrow(length=unit(0.13,"cm"), ends="last")) +
  annotate("segment", x=E_R, xend=E_R, y=B_M, yend=E_M,
           color="#2E7D32", linewidth=0.8,
           arrow=arrow(length=unit(0.13,"cm"), ends="last")) +
  # Labels
  annotate("text", x=A_R+6,  y=A_M+7,  label="A", fontface="bold", size=3.4, color="#012169") +
  annotate("text", x=B_R+5,  y=B_M-8,  label="B", fontface="bold", size=3.4, color="#012169") +
  annotate("text", x=E_R-14, y=E_M+7,  label="E", fontface="bold", size=3.4, color="#C8952A") +
  # Price line labels (near right edge)
  annotate("text", x=212, y=100*sqrt(2)-0.5*212+7, label="P[a]", parse=TRUE,
           size=3.0, color="grey45", hjust=0) +
  annotate("text", x=212, y=Pw_c-212-7, label="P[w]", parse=TRUE,
           size=3.0, color="#C8952A", hjust=0) +
  # IC labels (where curves are clearly visible)
  annotate("text", x=130, y=U2/130+8,  label="IC[2]", parse=TRUE, size=2.8, color="#2E7D32") +
  annotate("text", x=130, y=U1/130-9,  label="IC[1]", parse=TRUE, size=2.8, color="grey55") +
  # Trade labels
  annotate("text", x=(B_R+E_R)/2, y=B_M-10,
           label="Exports Rice", size=2.5, hjust=0.5, color="#2E7D32") +
  annotate("text", x=E_R+4, y=(B_M+E_M)/2,
           label="Imports\nMachines", size=2.5, hjust=0, color="#2E7D32") +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim = c(0, 222), ylim = c(0, 122)) +
  labs(title = "India (Labor-Abundant)",
       x = "Rice (labor-intensive)", y = "Machines (capital-intensive)") + th

# ── USA panel ─────────────────────────────────────────────────────────────────
p_usa <- ggplot() +
  # PPF  (M = 200√(1−(R/100)²))
  geom_line(data = data.frame(R = seq(0.5,100,len=400),
                              M = 200*sqrt(1-(seq(0.5,100,len=400)/100)^2)),
            aes(R, M), color = "#B22234", linewidth = 1.4) +
  # Pa*: M = 200√2 − 2R  (slope −2, machines are cheap at autarky)
  geom_line(data = data.frame(R = ru, M = pmax(0, 200*sqrt(2) - 2*ru)),
            aes(R, M), color = "grey45", linewidth = 0.8, linetype = "dotted") +
  # Pw: M = Pw_c − R  (same world price)
  ...
  geom_line(data = data.frame(R = ru, M = U2/ru), aes(R, M),
            color = "#2E7D32", linewidth = 0.8, linetype = "dotted") +
  # Points A*, B*, E*
  geom_point(aes(As_R, As_M), color = "#B22234", size = 2.8) +
  geom_point(aes(Bs_R, Bs_M), color = "#B22234", size = 2.8) +
  geom_point(aes(Es_R, Es_M), color = "#C8952A", size = 2.8) +
  # Trade triangle:
  #   USA EXPORTS Machines → vertical arrow from B* downward to E*_M level
  #   USA IMPORTS Rice     → horizontal arrow from B*_R rightward to E*_R
  annotate("segment", x=Bs_R, xend=Bs_R, y=Bs_M, yend=Es_M,
           color="#2E7D32", linewidth=0.8,
           arrow=arrow(length=unit(0.13,"cm"), ends="last")) +
  annotate("segment", x=Bs_R, xend=Es_R, y=Es_M, yend=Es_M,
           color="#2E7D32", linewidth=0.8,
           arrow=arrow(length=unit(0.13,"cm"), ends="last")) +
  # Labels
  annotate("text", x=As_R+4,  y=As_M+8,  label="A*", fontface="bold", size=3.4, color="#B22234") +
  annotate("text", x=Bs_R-13, y=Bs_M+6,  label="B*", fontface="bold", size=3.4, color="#B22234") +
  annotate("text", x=Es_R+4,  y=Es_M-9,  label="E*", fontface="bold", size=3.4, color="#C8952A") +
  # Price line labels
  annotate("text", x=96, y=200*sqrt(2)-2*96-9,  label="P[a]^'*'", parse=TRUE,
           size=3.0, color="grey45", hjust=0) +
  annotate("text", x=96, y=Pw_c-96+7,            label="P[w]",     parse=TRUE,
           size=3.0, color="#C8952A", hjust=0) +
  # IC labels
  annotate("text", x=78, y=U2/78+8,  label="IC[2]^'*'", parse=TRUE, size=2.8, color="#2E7D32") +
  annotate("text", x=78, y=U1/78-9,  label="IC[1]^'*'", parse=TRUE, size=2.8, color="grey55") +
  # Trade labels
  annotate("text", x=Bs_R-4,        y=(Bs_M+Es_M)/2,
           label="Exports\nMachines", size=2.5, hjust=1, color="#2E7D32") +
  annotate("text", x=(Bs_R+Es_R)/2, y=Es_M-10,
           label="Imports Rice", size=2.5, hjust=0.5, color="#2E7D32") +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
  coord_cartesian(xlim = c(0, 120), ylim = c(0, 213)) +
