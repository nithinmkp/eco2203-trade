---
title: "Modern Theories: Heckscher-Ohlin & Lewis Dual-Sector Model"
...
# Part I: Why Do Opportunity Costs Differ? {background-color="#1a3a5c"}
...
Ricardo assumed labour productivity differences but gave no explanation for *why* they exist.
:::
::: {.column width="40%"}
**The Heckscher-Ohlin answer:**

Opportunity costs differ because countries differ in their **endowments** of factors of production:

- Labour ($L$)
- Capital ($K$)
- Land ($T$)

And goods differ in **factor intensity** — how much of each factor they require.
:::
:::
...
- **Two countries:** India ($I$) and USA ($U$)
- **Two goods:** Rice ($R$) and Machines ($M$)
...
Rice is **labour-intensive** relative to Machines if:
...
:::
::: {.column width="45%"}
**Factor abundance definition:**

India is **labour-abundant** relative to the USA if:
...
i.e., labour is *relatively cheaper* in India.
:::
:::

# Part II: The Heckscher-Ohlin Theorem {background-color="#1a3a5c"}
...
```{r}
#| label: fig-edgeworth
#| fig-cap: "Edgeworth Box: Factor Allocation Between Two Industries\nSource: Author's illustration."
#| fig-width: 7
#| fig-height: 5
library(ggplot2)
...
# Rice isoquant through E (approx Cobb-Douglas: L^0.7 * K^0.3 = const)
# E at (L=30, K=12): U_R = 30^0.7 * 12^0.3 = 19.8
iso_L_R <- seq(5, 55, by=1)
iso_K_R <- (19.8 / iso_L_R^0.7)^(1/0.3)

# Machine isoquant through E in rotated coords (L_M=70, K_M=48)
# E at L_M=70, K_M=48: L_M^0.3 * K_M^0.7 = 70^0.3*48^0.7 = 58.0
iso_L_M_rot <- seq(50, 95, by=1)
iso_K_M_rot <- (58.0 / iso_L_M_rot^0.3)^(1/0.7)
iso_L_M <- 100 - iso_L_M_rot
iso_K_M <-  60 - iso_K_M_rot

ggplot() +
  annotate("rect", xmin=0, xmax=100, ymin=0, ymax=60,
           fill=NA, color="black", linewidth=1) +
  geom_line(data=data.frame(L=cc_L, K=cc_K),
            aes(x=L, y=K), color="#012169", linewidth=1.5) +
  geom_line(data=data.frame(L=iso_L_R, K=iso_K_R),
            aes(x=L, y=K), color="#012169", linewidth=0.8, linetype="dashed") +
  geom_line(data=data.frame(L=iso_L_M, K=iso_K_M),
            aes(x=L, y=K), color="#B9975B", linewidth=0.8, linetype="dashed") +
  geom_point(aes(x=30, y=12), size=4, color="darkgreen") +
  annotate("text", x=33, y=13, label="E (efficient alloc.)",
           size=3.5, hjust=0, color="darkgreen") +
  annotate("text", x=3,  y=3,  label="O[R]", parse=TRUE, size=4, color="#012169") +
  annotate("text", x=95, y=56, label="O[M]", parse=TRUE, size=4, color="#B9975B") +
  annotate("text", x=50, y=-6, label="Labour allocated to Rice",
           size=3.5, hjust=0.5) +
  labs(title="Contract curve: all Pareto-efficient factor allocations",
       subtitle=expression(paste("At E: ", MPL[R]/MPK[R], " = ", MPL[M]/MPK[M])),
       x="Labour (L)", y="Capital (K)") +
  coord_cartesian(xlim=c(-2,105), ylim=c(-8,65)) +
...
# Part III: Three Key Theorems {background-color="#1a3a5c"}
...
Wages in India $\approx$ \$2/hour; USA $\approx$ \$30/hour. FPE clearly does *not* hold in practice.
...
:::
::: {.column width="50%"}
**Applied to US opening to trade:**

$$P_M \uparrow \Rightarrow r \uparrow\uparrow, \quad w \downarrow$$

- US capital owners gain
- US workers lose → **political economy of protectionism!**
:::
:::
...
- Capital in capital-abundant countries → **pro-trade**
...
This is Stolper-Samuelson in action — higher output prices benefit agricultural labour.
:::
::: {.column width="45%"}
**The compensation principle:**

Even when aggregate welfare rises, there are **distributional losers**.

A country could in principle use the gains to compensate losers, but in practice:

1. Compensation schemes are costly to administer
2. Political constraints prevent redistribution
3. Short-run adjustment costs fall unevenly

This is why **trade adjustment assistance** (TAA) programs exist in many countries.
:::
:::
...
Expanding Rice output (to absorb new labour) requires withdrawing capital from Machines, even though capital endowment is unchanged. At constant prices, the only way to re-equate factor ratios is to pull capital into Rice and contract Machines.
:::
::: {.column width="50%"}
**India application:**

India's rural-to-urban migration (labour inflow to manufacturing) can be seen as a Rybczynski effect in reverse:
...
# Part IV: The Leontief Paradox and Extensions {background-color="#1a3a5c"}
...
US labour is highly skilled — skill-adjusted, US workers embody large amounts of human capital. US exports are "labour-intensive" in unskilled labour but capital-intensive in human capital.

$$\text{Effective } K/L = \frac{K + K_H}{L}$$

**2. Factor intensity reversals:**

Same good can be labour-intensive in one country and capital-intensive in another (different factor prices → different techniques).

**3. Natural resource abundance:**

US imports include oil and raw materials — these are capital-intensive. Excluding natural resource industries resolves much of the paradox.
:::
::: {.column width="45%"}
**4. Demand bias:**

US consumers have stronger preference for capital-intensive goods (high income elasticity of demand for manufactures). Demand effects can offset supply-side comparative advantage.

**5. Trade barriers:**

US tariff structure historically protected labour-intensive industries (agriculture, textiles), distorting trade flows away from comparative advantage.

**Modern verdict (Trefler 1995):**

Allowing for **technology differences** and **factor productivity** differences across countries reconciles most of the H-O predictions with data — the "mystery of the missing trade" is partly resolved.
:::
:::

# Part V: The Lewis Dual-Sector Model and Agricultural Trade {background-color="#1a3a5c"}
...
- **Traditional sector (agriculture):** large labour surplus, low marginal product of labour ($MPL_A \approx 0$)
- **Modern sector (industry/services):** rising capital intensity, higher wages

**Lewis's key assumption:**

$$MPL_A < \bar{w}_{\text{subsistence}}$$

There is a pool of "surplus labour" in agriculture that can be absorbed by industry **without raising agricultural wages**.
...
1. Agricultural labour supply is highly elastic → low rural wages
2. Low wages → comparative advantage in labour-intensive agricultural exports
3. As industrialisation proceeds → rural wages rise → agricultural comparative advantage erodes
4. Eventually: Rybczynski-type shift away from agricultural exports

**East Asia's trajectory:**

Japan (1950s–70s) → South Korea (1970s–90s) → China (2000s–) each followed this path: strong agricultural/labour-intensive exports early, shifting to capital-intensive manufactures.
:::
:::
...
**Combined implication for India:**

India currently sits at an early Lewis stage:

- Large agricultural labour surplus ($\approx 40$% of workforce in agriculture)
- Low rural wages → strong comparative advantage in labour-intensive agri exports
- As structural transformation proceeds, advantage will erode (as it did in South Korea)
:::
::: {.column width="45%"}
**India's agricultural export performance (FY2024):**

- Total agri-food exports: **$43.7 billion**
- Rice: $\approx$ \$10.4 billion (before export ban)
- Marine products: \$7.6 billion
- Spices: \$3.7 billion
- Cotton: \$2.1 billion

**H-O explains:** labour-intensive crops dominate

**Lewis explains:** low rural wages underpin price competitiveness

**Policy implication:** India should invest in agricultural productivity to maintain competitiveness even as wages rise — moving up the value chain rather than defending low-wage advantage.
:::
:::

# Part VI: Summary and Looking Ahead {background-color="#1a3a5c"}
...
2. **Factor Price Equalisation:**
$$w^I = w^U, \quad r^I = r^U$$
(free trade substitutes for factor mobility)

3. **Stolper-Samuelson:**
$$\hat{P}_R > 0 \Rightarrow \hat{w} > \hat{P}_R > 0 > \hat{r}$$
(trade has stark distributional effects)
...
**What H-O cannot explain:**

- Intra-industry trade (Germany ↔ France in cars)
- Trade between similar countries
- Scale economies and first-mover advantages
→ **New Trade Theory** (Lecture 6): Krugman (1979), Helpman-Krugman (1985)
:::
:::
