...
# Rice:     Q_R = L_R^0.7 * K_R^0.3  (labour-intensive)
# Machines: Q_M = L_M^0.3 * K_M^0.7  (capital-intensive)
# Contract curve from MRTS_R = MRTS_M => K_R = 540*L_R / (4900 - 40*L_R)
L_cc <- seq(1, 99, by = 0.5)
K_cc <- 540 * L_cc / (4900 - 40 * L_cc)

# Point E exactly on the contract curve at L_R = 50
L_E <- 50
K_E <- 540 * L_E / (4900 - 40 * L_E)   # = 9.31

# Rice isoquant through E
Q_R_E <- L_E^0.7 * K_E^0.3
iL_R  <- seq(23, 96, by = 0.5)
iK_R  <- (Q_R_E / iL_R^0.7)^(1 / 0.3)
df_R  <- subset(data.frame(L = iL_R, K = iK_R), K >= 0 & K <= 60)

# Machines isoquant through E (drawn in original coords)
Q_M_E <- (100 - L_E)^0.3 * (60 - K_E)^0.7
iLM   <- seq(34, 99, by = 0.5)
iKM   <- (Q_M_E / iLM^0.3)^(1 / 0.7)
...
ggplot() +
  ...
  annotate("text", x = 54, y = 12, hjust = 0, size = 3.5,
  ...
  # Isoquant labels
  annotate("text", x = 82, y = 2.5, hjust = 0, size = 3,
           label = "Rice isoquant", color = "#012169") +
  annotate("text", x = 5, y = 40, hjust = 0, size = 3,
           label = "Machines\nisoquant", color = "#B9975B") +
  # Contract curve label
  annotate("text", x = 64, y = 27, hjust = 0, size = 3.5, fontface = "bold",
           label = "Contract\ncurve", color = "#012169") +
  labs(
