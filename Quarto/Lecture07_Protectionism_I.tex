...
\textbf{But reality differs:}

Most countries protect agriculture. India levies average tariffs of
\textbf{\textasciitilde36\%} on farm goods. Why?
\end{column}
\end{columns}
...
\tightlist
\item
  {Specific tariff}: fixed amount per unit \emph{e.g.} ₹500 per tonne of
  wheat
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture07_Protectionism_I_files/figure-beamer/fig-free-trade-1.pdf}}
...
\textbf{Free trade equilibrium at \(P_w\):}
...
\textbf{After tariff \(t\): domestic price → \(P_w + t\):}
...
\textbf{Triangle \emph{B}} = production distortion loss

\textbf{Triangle \emph{D}} = consumption distortion loss
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture07_Protectionism_I_files/figure-beamer/fig-tariff-welfare-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture07_Protectionism_I_files/figure-beamer/fig-india-tariffs-1.pdf}}
...
\tightlist
\item
  Allows \emph{some} imports cheaply → food security
\item
  Discourages \emph{large} imports → protects producers
\end{itemize}
\end{column}
...
India imports \textbf{\textasciitilde14 million tonnes/year} of edible
oils --- 60\% of consumption. Cannot substitute quickly.
...
\begin{columns}[T]
\begin{column}{0.55\linewidth}
\textbf{Nominal tariff} = tariff on the final good.

\textbf{ERP} = protection on \emph{value added} in production,
accounting for tariffs on \emph{inputs}.
...
The gap between domestic and world price × quota volume = \textbf{quota
rent}.

Who captures the rent? - If licences \textbf{auctioned} → government -
If licences \textbf{given away} → importers (or exporters under VER)
\end{column}
\end{columns}
\end{block}
\end{frame}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture07_Protectionism_I_files/figure-beamer/fig-quota-1.pdf}}
...
\begin{columns}[T]
\begin{column}{0.55\linewidth}
\textbf{When quota rent goes to foreigners:}
...
\item
  The domestic economy loses the tariff-equivalent rectangle \emph{C}
\item
  Net welfare loss = \(B + C + D\) \textgreater{} tariff welfare loss of
  \(B + D\)
\end{itemize}

\textbf{When quota rent stays domestic:}

\begin{itemize}
\tightlist
\item
  If licences are \emph{auctioned} to domestic importers
\item
...
\begin{column}{0.45\linewidth}
\textbf{Implication for policy:}

If quota rents accrue to foreign exporters, quotas are \textbf{worse
than equivalent tariffs} --- the domestic economy loses both the DWL and
the revenue.

This is why economists generally prefer tariffs over quotas, and why
WTO's GATT Article XI prohibits quotas.
\end{column}
\end{columns}
\end{block}
\end{frame}
...
\begin{column}{0.45\linewidth}
\textbf{Post-2001 shift:}

India replaced QRs with \textbf{high tariffs} --- achieving similar
protection through a WTO-consistent instrument.
...
\begin{column}{0.45\linewidth}
\textbf{India's case (palm oil):}

India = 15\% of world palm oil demand. Malaysian/Indonesian export
supply somewhat inelastic.

→ India can depress world palm oil prices with a tariff → Terms of trade
gain partially offsets DWL
...
\textbf{Temporary} protection allows the industry to mature → eventually
competitive.
...
\textbf{Effect:} Like a quota but the \emph{quota rent accrues to the
exporter} (not the importer government).

\textbf{WTO:} VERs are now illegal under Article 11 of WTO Safeguards
Agreement (post-Uruguay Round).
\end{column}

\begin{column}{0.45\linewidth}
\textbf{India-adjacent example:}

India's 2023 rice export restrictions raised world prices ---
effectively a VER from the perspective of importing nations like
Bangladesh, Benin, and Senegal.

India's rice export controls demonstrated India's market power as the
world's dominant rice exporter.
\end{column}
\end{columns}
\end{block}
\end{frame}
...
\item
  ...
  quality standards
\item
  \textbf{Licensing requirements}
\item
  \textbf{Customs procedures and delays}
\item
  \textbf{State trading monopolies}
\end{itemize}
\end{column}

\begin{column}{0.45\linewidth}
\textbf{The ``new protectionism'':}

As tariffs have fallen under WTO, NTBs have risen to replace them.

India's rejection of GM soybean imports (SPS), EU's pesticide MRLs on
Indian spices, and USA's countervailing duties on Indian shrimp are all
NTB-related disputes.
