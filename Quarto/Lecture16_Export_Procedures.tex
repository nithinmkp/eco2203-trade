...
\title[L16 \textbar{} Export Procedures]{Lecture 16: Export Procedures
and Documentation}
...
\tightlist
\item
  \textbf{Commercial Invoice} --- product, quantity, price, HSN/IEC;
  basis for customs valuation
\item
  \textbf{Packing List} --- box-by-box weight and dimensions
\item
  \textbf{Transport document} --- \textbf{B/L} (sea) or \textbf{AWB}
  (air)
\item
  \textbf{Certificate of Origin (CoO)} --- used for FTA preferential
  tariffs
\item
  \textbf{Key idea:} \textbf{B/L is negotiable} (can be
...
\tightlist
\item
  \textbf{Phytosanitary certificate (PPQS)} --- plant products are
  pest-free
\item
  \textbf{Health/inspection certificate (EIC)} ---
  meat/marine/organic/dairy
\item
  \textbf{Fumigation certificate} --- pulses/oilseeds; treatment details
\item
  \textbf{Shipping Bill (ICEGATE)} --- customs clearance + \textbf{LEO}
\item
  \textbf{EDPMS (RBI)} --- links shipment to forex realization
\end{itemize}
...
\begin{frame}
\begin{block}{Document flow: a typical agri export}
\phantomsection\label{document-flow-a-typical-agri-export}
\begin{enumerate}
\tightlist
\item
  Contract signed + invoice prepared
\item
  Buyer arranges payment (often via \textbf{LC})
\item
  Shipment booked → \textbf{B/L or AWB} issued
\item
  Certificates obtained + Shipping Bill filed → \textbf{LEO}
\item
  Documents presented to bank → payment + \textbf{EDPMS} closure
\end{enumerate}
\end{block}
\end{frame}
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture16_Export_Procedures_files/figure-beamer/fig-export-credit-1.pdf}}

}

\caption{Pre-shipment credit dipped during
...
\pandocbounded{\includegraphics[keepaspectratio]{Lecture16_Export_Procedures_files/figure-beamer/fig-export-mode-1.pdf}}

}

\caption{Sea freight dominates at
