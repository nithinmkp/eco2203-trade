...
# Part I: The Problem with Absolute Advantage {background-color="#1a3a5c"}
...
The United States is more productive than Bangladesh in *both* textiles *and* aircraft.

Yet the US imports textiles from Bangladesh.

**Why?** → Enter David Ricardo (1817).
:::
:::
...
A lawyer who types faster than her secretary still benefits from hiring the secretary — because the lawyer's time is more valuably spent on legal work.

The same logic applies to nations.
:::
:::

# Part II: The Ricardian Model — Formal Setup {background-color="#1a3a5c"}
...
::: {.columns}
::: {.column width="50%"}
**Assume:** each country has $L = 100$ workers.

One worker per day produces:
...
So the labour coefficients are:

$$a_{LR}^{I} = \tfrac{1}{2}, \quad a_{LWh}^{I} = 1$$
$$a_{LR}^{W} = \tfrac{1}{3}, \quad a_{LWh}^{W} = \tfrac{1}{3}$$

The **World** has absolute advantage in **both** goods (lower $a$).
:::
::: {.column width="50%"}
**Opportunity costs:**

To produce one more unit of **Rice**, India must give up:

$$OC_R^{I} = \frac{a_{LR}^{I}}{a_{LWh}^{I}} = \frac{1/2}{1} = \frac{1}{2} \text{ Wheat}$$

To produce one more unit of Rice, the World must give up:

$$OC_R^{W} = \frac{a_{LR}^{W}}{a_{LWh}^{W}} = \frac{1/3}{1/3} = 1 \text{ Wheat}$$

**Comparison:** $OC_R^{I} = \tfrac{1}{2} < OC_R^{W} = 1$

$\Rightarrow$ **India has comparative advantage in Rice.**
:::
:::
...
# Part III: Terms of Trade and Gains from Trade {background-color="#1a3a5c"}
...
The trade possibilities line starts at full specialisation point (0, 200) with slope $= -1/1.5 \approx -0.67$ (less steep than PPF slope $-2$).
...
rice_trade <- pmax(0, 200 - (200/133)*wheat_seq)
...
ggplot(df_ppf, aes(x=wheat)) +
  ...
  geom_point(aes(x=80, y=93), color="#B9975B", size=4) +
  annotate("text", x=84, y=93,
           label="Post-trade consumption\n(80Wh, 93R) — above PPF!",
  ...
  annotate("segment", x=50, y=100, xend=80, yend=93,
           arrow=arrow(length=unit(0.25,"cm")), color="darkgreen", linewidth=1) +
  annotate("text", x=58, y=112, label="Gains from\ntrade!",
           color="darkgreen", size=3.5, fontface="bold") +
  annotate("text", x=5, y=170, label="Trade line\n(slope = -P_R/P_Wh)",
           color="#B9975B", size=3, hjust=0) +
...
India produces and consumes on the PPF, say at:
$$(Wh, R) = (50, 100)$$

**After trade** (at $P_{Wh}/P_R = 1.5$):

India fully specialises → 200 Rice, trades $107$ Rice for $\approx 80$ Wheat:

$$(Wh, R) = (80, 93)$$

**Welfare gain:** more Wheat *and* a larger combined bundle.

The **real wage** of Indian workers rises: each hour of labour now commands more consumption goods.
:::
::: {.column width="45%"}
**The algebra:**

India's income at world prices (fully specialised):

$$Y^I = P_R \times 200$$

Budget constraint under trade:

$$P_R \cdot R + P_{Wh} \cdot Wh = P_R \times 200$$

$$R + 1.5 \cdot Wh = 200$$

Compare with autarky constraint:

$$R + 2 \cdot Wh = 200$$

The trade budget line is **flatter** → larger feasible set.
:::
:::

# Part IV: Indifference Curves and Equilibrium {background-color="#1a3a5c"}
...
rice_ppf   <- pmax(0, 200 - 2*wheat_seq)
rice_trade <- pmax(0, 200 - wheat_seq)
rice_ic1   <- 5000 / wheat_seq
rice_ic2   <- 9600 / wheat_seq
...
ggplot(df, aes(x=wheat)) +
  ...
  geom_point(aes(x=80,  y=120), color="#B9975B", size=3.5) +
  annotate("text", x=52, y=94,  label="E1 (autarky)",   size=3, hjust=0) +
  annotate("text", x=82, y=114, label="E2 (free trade)", size=3, hjust=0) +
...
# Part V: Assumptions, Extensions, and Critique {background-color="#1a3a5c"}
...
Golub & Hsieh (2000) and Costinot et al. (2012) find strong evidence that labour productivity differences predict export patterns across countries.

**Limitations:**

- Predicts complete specialisation (rarely observed)
- Says nothing about *who within a country* gains or loses
- Cannot explain intra-industry trade
:::
::: {.column width="45%"}
**Extensions:**

- **Many goods:** a continuum of goods ranked by comparative advantage — a "Dornbusch-Fischer-Samuelson" chain
- **Many countries:** bilateral comparisons still apply
- **Technology changes:** productivity growth shifts the chain, altering trade patterns over time
:::
:::

# Part VI: India — Revealed Comparative Advantage {background-color="#1a3a5c"}
...
- Export value: ₹60,000+ crore in FY2023
- Destinations: Bangladesh, Indonesia, Benin, Senegal, UAE

**Why does Ricardo's model fit?**

- Rice cultivation is labour-intensive — India's abundant factor
- Labour productivity in Indian rice farming (output per worker) is competitive even though absolute productivity lags behind Vietnam/Thailand
- **Relative** labour productivity in rice > relative labour productivity in capital-intensive goods

**Policy tension:**

September 2023: India imposed a **25% export duty** on non-Basmati white rice to contain domestic inflation → global rice prices spiked 15%.
:::
::: {.column width="45%"}
**Ricardian lens on the export ban:**

When India restricted rice exports:

$$P_R^{\text{world}} \uparrow \Rightarrow \text{India's ToT deteriorate for future exporters}$$

The ban is welfare-reducing under comparative advantage theory: India surrenders gains from trade to achieve food security goals.

**This is the central policy tension in agricultural trade:**

- Static efficiency ↔ food security
- Comparative advantage ↔ strategic reserves
- Export competitiveness ↔ domestic price stability
:::
:::
...
2. Comparative advantage condition:
$$\frac{a_{LR}^{I}}{a_{LWh}^{I}} < \frac{a_{LR}^{W}}{a_{LWh}^{W}}$$

3. Gains from trade: consumption bundle moves **beyond the PPF**

4. Terms of trade must lie between autarky opportunity costs:
...
**What Ricardo cannot explain:**

- *Why* labour productivity differs (→ H-O model)
- Income distribution effects (→ Stolper-Samuelson)
- Intra-industry trade (→ new trade theory, Lecture 6)

**Next lecture:** Where do comparative advantages come from? Factor endowments and the Heckscher-Ohlin model.
:::
:::
